defmodule Pummpcomm.Driver.ReadBgTargets do
  ...
  test "ReadBgTargets results in a Map", %{pump_serial: pump_serial} do
    ...
    assert %{units: _, targets: _} = ReadBgTargets.decode(context.response)
  end
  ...
  test "ReadBgTargets.decode has correct information" do
    ...
    assert %{units: "mg/dL", targets: expected_targets} == ReadBgTargets.decode(%Response{opcode: 0x9F, data: response_data})
  end
end
