defmodule Pummpcomm.Driver.ReadTempBasalTest do
  ...
  test "ReadTempBasal results in Map", %{pump_serial: pump_serial} do
    ...
    assert {:ok, %{type: _, units_per_hour: _, duration: _}} = ReadTempBasal.decode(context.response)
  end
  ...
  test "ReadTempBasal decodes unset temp_basal" do
    ...
    assert {:ok, %{type: :absolute, units_per_hour: 0.000, duration: 0}} = result
  end
  ...
  test "ReadTempBasal decodes absolute temp_basal of 0" do
    ...
    assert {:ok, %{type: :absolute, units_per_hour: 0.0, duration: 60}} = result
  end
  ...
  test "ReadTempBasal decodes positive absolute temp_basal" do
    ...
    assert {:ok, %{type: :absolute, units_per_hour: 0.025, duration: 30}} = result
  end
  ...
  test "ReadTempBasal decodes percent temp_basal" do
    ...
    assert {:ok, %{type: :percent, units_per_hour: 95, duration: 30}} = result
  end
end
