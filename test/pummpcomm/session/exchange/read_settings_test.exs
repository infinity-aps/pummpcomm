defmodule Pummpcomm.Driver.ReadSettingsTest do
  ...
  test "ReadSettings results in a Map", %{pump_serial: pump_serial} do
    ...
    assert %{max_bolus: _, max_basal: _, selected_basal_profile: _, insulin_action_curve_hours: _} = ReadSettings.decode(context.response)
  end
  ...
  test "ReadSettings decodes the correct information for smaller format" do
    ...
    assert %{max_bolus: 20.0, max_basal: 2.0, selected_basal_profile: :profile_a, insulin_action_curve_hours: 3} == ReadSettings.decode(%Response{opcode: 0xC0, data: response_data})
  end
end
