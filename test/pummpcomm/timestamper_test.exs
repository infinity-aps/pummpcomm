defmodule Pummpcomm.TimestamperTest do
  ...
  test "correctly assigns relative timestamps" do
    ...
    assert {:sensor_glucose_value, %{sgv: 104, timestamp: ~N[2016-02-08 20:49:00]}} =
             Enum.at(timestamped_events, 0)

    assert {:sensor_glucose_value, %{sgv: 138, timestamp: ~N[2016-02-08 20:54:00]}} =
             Enum.at(timestamped_events, 1)

    assert {:sensor_timestamp, %{timestamp: ~N[2016-02-08 20:54:00]}} =
             Enum.at(timestamped_events, 2)
  end
  ...
  test "it timestamps forward when there are only relative events since the last last_rf reference sensor timestamp" do
    ...
    assert {:sensor_timestamp, %{timestamp: ~N[2016-02-08 20:54:00], event_type: :last_rf}} =
             Enum.at(timestamped_events, 0)

    assert {:sensor_glucose_value, %{sgv: 104, timestamp: ~N[2016-02-08 20:59:00]}} =
             Enum.at(timestamped_events, 1)

    assert {:sensor_glucose_value, %{sgv: 138, timestamp: ~N[2016-02-08 21:04:00]}} =
             Enum.at(timestamped_events, 2)
  end
  ...
  test "it does not timestamp forward when the most recent sensor timestamp is something other than rf" do
    ...
    assert {:sensor_timestamp, %{timestamp: ~N[2016-02-08 20:54:00], event_type: :gap}} =
             Enum.at(timestamped_events, 0)
