defmodule Pummpcomm.Monitor.BloodGlucoseMonitor do
  ...
  defp fetch_and_filter_page(_, page_number, sensor_values, oldest_allowed, lowest_page_allowed) when page_number < lowest_page_allowed do
  ...
  defp fetch_and_filter_page(cgm, page_number, sensor_values, oldest_allowed, lowest_page_allowed) do
    ...
    case Cgm.needs_timestamp?(values) do
      true ->
        ...
        fetch_and_filter_page(cgm, page_number, sensor_values, oldest_allowed, lowest_page_allowed)
      false ->
        newest_first_values = Enum.reverse(values)
        {oldest_reached, sensor_values} = newest_first_values
        ...
        case oldest_reached do
          ...
          false -> fetch_and_filter_page(cgm, page_number - 1, sensor_values, oldest_allowed, lowest_page_allowed)
        end
    end
  end
