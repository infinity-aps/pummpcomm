defmodule Pummpcomm.Monitor.HistoryMonitor do
  ...
  def get_pump_history(minutes_back, timezone) do
    ...
    Logger.info(fn ->
      "Searching until we find a history entry older than #{inspect(oldest_allowed)}"
    end)

    {:ok, fetch_and_filter_page(0, [], oldest_allowed, 5)}
  end

  defp fetch_and_filter_page(page_number, history_events, oldest_allowed, highest_page_allowed)
       when page_number > highest_page_allowed do
    Logger.warn(fn ->
      "Reached max page fetches before finding an event older than #{inspect(oldest_allowed)}"
    end)
